...
class RegistrationService
{
    ...
    public function registerWithStripe(
        ...
        return DB::transaction(function () use ($payload, $stripeCustomerId, $stripeSubscriptionId, $status, $stripeSubscriptionStatus): RegistrationConfirmation {
            $adminData = $payload['admin'];
            $companyData = $payload['company'];
            $subscriptionData = $payload['subscription'];
            $workers = $payload['workers'] ?? [];
            ...
            $plan = Plan::query()->where('name', $subscriptionData['plan'])->firstOrFail();
            $period = (string) $subscriptionData['billing_period'];
            $trialEnabled = $status === Subscription::STATUS_TRIALING;

            $subscription = $this->companySubscriptionService->syncBusinessSubscription(
                $company,
                $plan->name,
                $period,
